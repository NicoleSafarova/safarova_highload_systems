...
*Keepalived обеспечивает резервирование через создание виртуального IP-адреса, который автоматически перемещается между серверами при сбоях. Master-узел активно обрабатывает трафик, а Backup-узлы постоянно мониторят его состояние через протокол.
