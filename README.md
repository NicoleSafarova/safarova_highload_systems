...
| Компонент системы | Технологии | Избыточности | Механизм резервирования | Восстановление при сбое |
|-------------------|------------|----------------------|-------------------------|------------------------|
| **Глобальная балансировка** | BGP Anycast | 2 ДЦ | Распределение по 2 ДЦ, хотя полностью хватает 1 | Автоперенаправление на ближайший ДЦ |
| **L7 Балансировщики** | NGINX  | N+1 на дата-центр | Горизонтальное масштабирование | Автозамена неисправных нод |
| **Микросервисы** | Kubernetes + Docker | 3+ реплики на сервис | Обеспечение избыточности за счет распределения подов по узлам кластера с автоматическим перезапуском на резервных узлах при сбоях. Реализуется через Replicas, Deployments и системы бэкапа. | Самовосстановление подов |
| **Хранилище файлов** | S3-совместимое (Yandex Object Storage + MinIO(резерв)) | 2 копии данных | Репликация между ДЦ | Автоматическое переключение и восстановление из соседнего ДЦ |
| **Базы данных** | PostgreSQL + Patroni | 1 реплика на шард | Синхронные/асинхронные реплики | Автоfailover через Patroni |
| **Кэш данных** | Redis Cluster | 1 реплика на мастер | Мониторинг через Sentinel | Автоfailover реплик |
| **Аналитика** | ClickHouse | RF=2 между ДЦ | Репликация через Zookeeper | Автосинхронизация данных |
| **Очереди заказов** | RabbitMQ Cluster | Зеркальные очереди | Копирование между нодами | Переключение на живые ноды |
| **Мониторинг** | Prometheus + Thanos | 2+ инстанса | Копирование метрик между регионами | Переход на резервный инстанс |
