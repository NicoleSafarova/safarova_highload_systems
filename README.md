...
|------------------|-------------------|----------|
| Векторный поиск | Основной поиск товаров | Векторный поиск = семантический поиск. То есть при вводе пользователем вещи, которую он хочет найти, с помощью нейросетей подбираются товары похожие по смыслу, которые искал пользователь, а не по ключевым словам. |

### Ключевые этапы работы векторного поиска:
### 1. Создание векторных представлений (Embeddings)

Последовательность:

-Текст разбивается на токены (слова, подсловья)

-Нейросетевая модель преобразует текст в вектор
   
   Пример для товара:
"Красное вечернее платье" = [0.23, -0.45, 0.78]

### 2. Построение векторного индекса
Цель: Организовать векторы для быстрого поиска

Методы индексации:

IVF (Inverted File Index) — кластеризация
PQ (Product Quantization) — сжатие векторов


### 3. Поиск ближайших соседей (составление топа похожих предметов)
Точное вычисление расстояния для кандидатов:

Косинусное сходство: cos(θ) = (A·B) / (||A|| × ||B||)

Евклидово расстояние: d = √Σ(aᵢ - bᵢ)²

Скалярное произведение: similarity = A·B
...
|--------|------------|----------|-------|------------|----------|
| **User Service** | 8200 |24600  |25 |32|Аутентификация, авторизация, профили пользователей. Высокий RPS из-за проверки сессий на каждом запросе|
| **Search Service** | 12731 | 38193 | 39|64|Векторный поиск, фильтрация. Требует много CPU для ранжирования и RAM для кэша результатов|
| **Catalog Service** | 4500 | 13500 | 14|32|Просмотр товаров, категорий. Умеренная нагрузка, кэширование на уровне приложения|
| **Order Service** | 1908 | 5724 |6 |16|Создание и управление заказами. Средняя нагрузка, важна консистентность данных|
| **Delivery Service** | 1200 | 3600 |3 |16|Расчет стоимости и сроков доставки. Интеграция с внешними API, геокодирование|
| **Cart Service** |6200  |  18600 | 19|32|Управление корзиной. Высокий RPS из-за частых обновлений|
| **Sellers Service** | 120 |360 |68 |32|Личный кабинет продавцов. Низкая нагрузка, в основном CRUD операции|
...
### Хостинг
Я выбрала для проекта совместное использование виртуальных машин и системы Kubernetes.

##### Собственное железо
| Ноды | Конфигурация | CPU (Ядра) | Колиство | RAM |Обоснование|
|--------|------------|----------|-------|------------|----------|
| **PostgreSQL** | 	2x6338/8x32GB/2xNVMe4T/2x25Gb/s |128  |3 |256|32 ядра × 3 = 96 ядер|
| **Redis Cluster (кэш)** | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 128 | 3|512|	148 ядер / 64 = 2.3|
| **Kubernetes** | 2x6338/8x32GB/2xNVMe2T/2x25Gb/s | 128 | 3|256|174 ядра / 64 = 2.7|
| **NGINX** | 2x6338/4x32GB/2xNVMe1T/2x25Gb/s | 128 |2|128|53 ядра / 64 = 0.8|
| **RabbitMQ** | 2x6338/8x32GB/4xNVMe2T/2x25Gb/s | 128 |2 |256|	29 ядер / 64 = 0.45|

##### Kubernetes
| Ноды | Конфигурация | CPU (Ядра) | Колиство | RAM |
|--------|------------|----------|-------|------------|
| **API Gateway** | 	2x6338/16x32GB/2xNVMe4T/2x25Gb/s |128  |2 |256|
| **Prometheus + Grafana** | 2x6338/32x32GB/2xNVMe4T/2x25Gb/s | 128 | 1|256|
| **CDN+Edge** | 	2x6338/8x32GB/4xNVMe2T/2x25Gb/s | 128 | 2|128|
| **K8s Worker Nodes** | 2x6338/16x32GB/2xNVMe4T/2x25Gb/s | 128 |3 |256|
