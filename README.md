...
|-------------|-----------------|-----------------|
| https://www.wildberries.ru/ | Москва | Повышает скорость загрузки сайта для пользователей  |
...
#### 3. Схема DNS балансировки 
2 ДЦ в Москве были выбраны по количеству заказов пользователей, и 1 ДЦ был выбран в Эектростали, потому что там располагается самый большой склад ВБ.

Схема:
Один ДЦ является резервным для другого.
1) Разделяем трафик на трафики юзеров и продавцов (разные IP). Трафик автоматически распределяется между двумя центрами по домену.
2) Если упадет ДЦ с пользователями, то так как есть второй ДЦ с продавцами, то с заказами все будет в порядке. К тому же продавцов в разы меньше пользователей, а значит часть пользователей удасться перенаправить в ДЦ с продавцами.
...
![Логическая схема БД](Highload systems.jpg)

| Таблица | Назначение | Особенности нагрузки |
|---------|------------|---------------------|
| **users** | Основная информация о пользователях |  95% чтение, горячие данные - активные пользователи |
| **user_profiles** | Доп. данные пользователей | Привязана к users, синхронные обновления |
| **user_addresses** | Адреса доставки | Частые чтения при оформлении заказов |
| **user_sessions** | Сессии пользователей |  TTL данные, высокие RPS, eventual consistency |
| **sellers** | Информация о продавцах |  Низкая частота изменений |
| **seller_contacts** | Контактные лица | Вторичные данные продавцов |
| **seller_bank_accounts** | Банковские счета |  Финансовые данные, высокая важность |
| **categories** | Категории товаров |   Иерархические данные, кэшируются |
| **products** | Основная информация о товарах |  Высокое чтение, кэширование, поисковые индексы |
| **product_attributes** | Характеристики товаров |   Фильтрация по атрибутам, полнотекстовый поиск |
| **product_images** | Изображения товаров |   CDN-кэширование, метаданные изображений |
| **product_prices** | История цен товаров |  Временные ряды, аналитические запросы |
| **orders** | Заголовки заказов |   Финансовые транзакции, ACID требования |
| **order_items** | Состав заказов |  Связана с orders, каскадные операции |
| **payments** | Платежи |   Высокая важность, идемпотентность |
| **sklad** | Склады |   Географические данные |
| **stocks** | Остатки товаров |  Высокая конкуренция, оптимистичные блокировки |
| **carts** | Корзины покупок |   Временные данные, TTL, eventual consistency |
| **cart_items** | Элементы корзины |  Частые обновления, сессионные данные |
| **product_search_cache** | Кэш поиска | TTL, инвалидация по времени/событиям |
| **user_recommendations** | Рекомендации |  Персональные данные, ML-обновления |
| **rate_limit_buckets** | Ограничение запросов |   Высокий RPS, TTL, статистические данные |
