...
|--------|------------|----------|-------|------------|----------|
| **User Service** | 8 200 |24600  |25 |32|Аутентификация, авторизация, профили пользователей. Высокий RPS из-за проверки сессий на каждом запросе|
| **Search Service** | 12 731 | 38 193 | 39|64|Векторный поиск, фильтрация. Требует много CPU для ранжирования и RAM для кэша результатов|
| **Catalog Service** | 4 500 | 13 500 | 14|32|Просмотр товаров, категорий. Умеренная нагрузка, кэширование на уровне приложения|
| **Order Service** | 1 908 | 5 724 |6 |16|Создание и управление заказами. Средняя нагрузка, важна консистентность данных|
| **Delivery Service** | 1 200 | 3 600 |3 |16|Расчет стоимости и сроков доставки. Интеграция с внешними API, геокодирование|
| **Cart Service** |6 200  |  18600 | 19|32|Управление корзиной. Высокий RPS из-за частых обновлений|
| **Sellers Service** | 120 |360 |68 |32|Личный кабинет продавцов. Низкая нагрузка, в основном CRUD операции|
| **BGP Anycast L4** | 45 000 | 135 000  |27|16|Глобальная балансировка на L3/L4. Меньше CPU чем NGINX |
| **NGINX L7** | 35 000 | 105000  |53|64|HTTP/HTTPS балансировка, SSL termination. Высокая нагрузка, кэширование, маршрутизация|
| **RabbitMQ** | 9 507 | 28 521  | 29|128|Очереди нотификации и оплаты через WB кошелек. Большой объем сообщений, требуется много RAM для буферизации|
| **Redis** | 49 305 | 147 915  |148|512|Кэш сессий, корзин, поиска. Экстремальный RPS, все данные в памяти, шардирование|
| **PostgreSQL** | 2 604 | 7 812  |32|256|Основное хранилище данных. Сложная нагрузка, репликация|
...
Синхронные запросы
Пользователь → BGP → L4 → L7 → API Gateway → Микросервисы → Базы данных


Асинхронные запросы
Микросервисы → RabbitMQ → Фоновые обработчики → Аналитика
