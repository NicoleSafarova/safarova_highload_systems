...
-Используются модели типа Sentence-BERT, Word2Vec, Doc2Vec
   
Пример для товара:
"Красное вечернее платье с кружевом" = [0.23, -0.45, 0.78, 0.12, -0.89, ..., 0.34]

2. Построение векторного индекса
Цель: Организовать векторы для быстрого поиска
Методы индексации:
IVF (Inverted File Index) — кластеризация 
HNSW (Hierarchical Navigable Small World) — иерархические графы
